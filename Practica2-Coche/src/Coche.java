 ...
 * velocidad.
 * 
 * @author Andoni Eguíluz Facultad de Ingeniería - Universidad de Deusto (2014)
 */
public class Coche {
	 ...
	 * 
	 * @param aceleracion
	 *            Incremento/decremento de la velocidad en pixels/segundo
	 * @param tiempo
	 ...
	 *            grados (-180 a +180) Considerando positivo giro antihorario,
	...
	public double fuerzaAceleracionAtras() {
		if (miVelocidad <= -350D)
			return FUERZA_BASE_ATRAS * ((miVelocidad + 500.0D)/150.0D);
		else if (miVelocidad <= -200.0D)
			return FUERZA_BASE_ATRAS;
		else if (miVelocidad <= 0.0D)
			return FUERZA_BASE_ATRAS * (miVelocidad / 200.0D * 0.7D + 0.3D);
		else if (miVelocidad <= 250.0D)
			return FUERZA_BASE_ATRAS * (miVelocidad / 250.0D * 0.55D + 0.3D);
		else
			return 850.0D;
	}
	...
	public String toString() {
		return piloto + " (" + posX + "," + posY + ") - " + "Velocidad: " + miVelocidad + " ## Dirección: "
				+ miDireccionActual;
	}
}
